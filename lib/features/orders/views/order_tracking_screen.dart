...
import 'package:najiz_go_express/features/orders/controllers/order_tracking_controller.dart';
...
class OrderTrackingScreen extends StatelessWidget {
  ...
  void _handleBack(OrderTrackingController controller, BuildContext context) {
    if (controller.vendorRejectionDetected.value ||
        controller.noDriverDetected.value) {
      return;
    }
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          leading: Obx(
            () => IconButton(
              onPressed: controller.vendorRejectionDetected.value ||
                      controller.noDriverDetected.value
                  ? null
                  : () async {
        ...
        body: SafeArea(
          child: Stack(
            children: [
              Obx(
                () => ListView(
                  ...
                  children: [
                    ...
                    if (controller.deliveryEta.value != null) ...[
                      const SizedBox(height: 10),
                      EstimatedDeliveryEtaCard(eta: controller.deliveryEta.value),
                    ],
              ...
              _NoDriverListener(controller: controller),
              _VendorRejectionListener(
...
class _NoDriverListener extends StatefulWidget {
  const _NoDriverListener({required this.controller});

  final OrderTrackingController controller;

  @override
  State<_NoDriverListener> createState() => _NoDriverListenerState();
}

class _VendorRejectionListener extends StatefulWidget {
  const _VendorRejectionListener({
  ...
  @override
  State<_VendorRejectionListener> createState() =>
      _VendorRejectionListenerState();
}

class _VendorRejectionListenerState extends State<_VendorRejectionListener> {
  bool _dialogShown = false;

  Future<void> _handleVendorRejection() async {
    if (!mounted || _dialogShown) return;
    _dialogShown = true;
    await showVendorOrderRejectedDialog(
      context,
      isStoreOrder: widget.isStoreOrder,
    );
    if (!mounted) return;
    AppRoutes.openHome(token: widget.controller.token, offAll: true);
  }

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      if (!widget.controller.vendorRejectionDetected.value) {
        return const SizedBox.shrink();
      }
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _handleVendorRejection();
      });
      return const SizedBox.shrink();
    });
  }
...
class _NoDriverListenerState extends State<_NoDriverListener> {
  bool _dialogShown = false;

  Future<void> _handleNoDriver() async {
    if (!mounted || _dialogShown) return;
    _dialogShown = true;
    await widget.controller.cancelOrderDueToNoDriver();
    if (!mounted) return;
    ...
    AppRoutes.openHome(token: widget.controller.token, offAll: false);
  }

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      if (!widget.controller.noDriverDetected.value) {
        return const SizedBox.shrink();
      }
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _handleNoDriver();
                      ...
                      onPressed: widget.controller.isSubmittingRating.value
                          ...
                          : () async {
                              ...
                              try {
                                ...
                                AppRoutes.openHome(token: widget.controller.token);
                                AppSnackbar.show(
                                  'tracking.thanks'.tr,
