    ...
    private fun init(call: MethodCall, result: Result) {
        ...
        if(_pusherInstance == null) {
            ...
            if (!options.isNull("auth")) {
                ...
                val contentType = headersMap["Content-Type"]?.lowercase() ?: ""
                val isJsonBody = contentType.contains("application/json")
                val encodedConnectionFactory = if (isJsonBody)
            ...
            if(!options.isNull("cluster")) {
                val cluster = options.getString("cluster")
